...
class ModuleProfiler:
    def __init__(self):
        ...
        self.latest_times = {}  # 新增：用于记录当前帧各模块的耗时
        self._starts = {}

    def clear_latest(self):
        """在每帧开始前清空上一帧的记录"""
        self.latest_times.clear()

    def start(self, module_name):
        self._starts[module_name] = time.perf_counter()

    def stop(self, module_name):
        if module_name in self._starts:
            elapsed = time.perf_counter() - self._starts[module_name]
            self.times[module_name] += elapsed
            self.counts[module_name] += 1
            self.latest_times[module_name] = elapsed  # 记录当前帧耗时
            del self._starts[module_name]

    def generate_report(self):
        """生成耗时统计表格"""
        print("\n" + "="*70)
        print(f"{'模块名称 (Module)':<25} | {'平均耗时 (ms)':<15} | {'总耗时 (s)':<10} | {'调用次数'}")
        print("-" * 70)
        # 按总耗时降序排列
        sorted_modules = sorted(self.times.items(), key=lambda x: x[1], reverse=True)
        for name, total_time in sorted_modules:
            total_ms = total_time * 1000
            avg_ms = total_ms / max(self.counts[name], 1)
            name_display = name[:24]
            # 总耗时按秒显示，平均耗时按毫秒显示
            print(f"{name_display:<25} | {avg_ms:<15.2f} | {total_time:<10.2f} | {self.counts[name]}")
        print("="*70 + "\n")
...
class VisionAnalysisService:
    ...
    def _detect_cell_phone_conf(self, person_crop):
        ...
        try:
            phone_results = self.phone_detector.predict(
                person_crop, verbose=False, classes=[CELL_PHONE_CLASS_ID],
                conf=0.05, device=self.yolo_device,
            )
        except Exception:
            return 0.0
    ...
    def _smooth_world_xy(self, track_filter_states, state_key, current_time_sec, observed_xy, quality_score, v_max_mmps, margin_mm, deadband_mm, outlier_confirm):
    ...
    def detect_person_from_image(self, image, camera_id=None, 
        ...
        self.profiler.clear_latest() # 新增：在开始处理每帧前清空上一帧的计时数据
        self.profiler.start("0_Total_Pipeline")
        self.profiler.start("1_State_And_Config_Init")
        ...
        frame = image
        persons_result = []
        self.profiler.stop("1_State_And_Config_Init")

        self.profiler.start("2_Person_Detection")
        if enable_tracking:
        ...
        self.profiler.stop("2_Person_Detection")
        ...
        if enable_tracking and has_person:
            ...
            self.profiler.stop("3_Person_ReID")
        ...
        for current_idx, (box, person_id, conf, track_id) in enumerate(zip(boxes, assigned_ids, assigned_confs, current_track_ids)):
            ...
            if uniformer_enabled and person_id != -1:
                self.profiler.start("4_Action_Uniformer")
                ...
                self.profiler.stop("4_Action_Uniformer")
            ...
            if enable_behavior:
                ...
                self.profiler.stop("5_Phone_Detection")
            ...
            if enable_face or enable_positioning:
                self.profiler.start("6_Pose_Estimation")
                ...
                self.profiler.stop("6_Pose_Estimation")

            if enable_face:
                self.profiler.start("7_Face_Recognition")
                    ...
                    if str(person_id) == str(detected_face_id):
                        if enable_tracking:
                            reidentifier.update_identity(
                                track_id, detected_face_id, base_crop, box=box,
                                all_boxes=boxes, frame=frame,
                            ...
                            if person_id != -1:
                                ...
                                if face_saved:
                                    reidentifier.save_unknown_reid_sample(
                                        track_id, person_id, base_crop, box=box,
                                        frame_shape=frame.shape, all_boxes=boxes,
                                        current_idx=current_idx,
                                    )
                self.profiler.stop("7_Face_Recognition")

            if enable_positioning:
                self.profiler.start("8_World_Coord_Calc")
                if keypoints_global is not None:
                ...
                if switch_from is not None and person_id not in (-1, None):
                    self._migrate_track_state(
                        track_filter_states, f"pid:{switch_from}", f"pid:{person_id}",
                        current_time_sec, self.ttl_keep_state,
                    )
                    state_key = f"pid:{person_id}"
                if person_id not in (-1, None) and track_id is not None:
                    self._migrate_track_state(
                        track_filter_states, f"track:{track_id}", f"pid:{person_id}",
                        current_time_sec, self.ttl_keep_state,
                    )
                    state_key = f"pid:{person_id}"
                ...
                if world_coords is not None:
                    quality_score = self._estimate_observation_quality(
                        detected_kp_count, keypoints_global, posture, keypoint_type,
                    )
                    smoothed_xy = self._smooth_world_xy(
                        track_filter_states, state_key, current_time_sec,
                        (float(world_coords[0]), float(world_coords[1])),
                        quality_score, self.v_max_mmps, self.margin_mm, self.deadband_mm, self.outlier_confirm,
                    )
                    if smoothed_xy is not None:
                        world_coords = (smoothed_xy[0], smoothed_xy[1])
                self.profiler.stop("8_World_Coord_Calc")
    ...
    # 视频输入及平均耗时计算入口
    # ==========================================
    def process_video(self, video_path, camera_id="video_01",
                      enable_face=True,
                      enable_behavior=False,
                      enable_uniformer=False,
                      enable_positioning=True,
                      enable_tracking=True,
                      output_txt="frame_process_times.txt"): # 新增: 文本输出路径
        """
        处理视频文件并统计全流程中各模块的平均耗时及单帧耗时。
        """
        print(f"\n>>> 开始处理视频: {video_path}")
        ...
        # 在处理新视频前，实例化一个新的 profiler 以防止与历史数据混淆
        self.profiler = ModuleProfiler()
        ...
        with open(output_txt, 'w', encoding='utf-8') as f:
            ...
            while True:
                ...
                # 核心修改：每隔五帧才取一帧进行整个流程 (第 1, 6, 11, 16... 帧)
                if frame_count % 5 != 1:
                    continue
                ...
                # 调用带耗时统计标记的处理管线
                self.detect_person_from_image(
                    image=frame,
                    camera_id=camera_id,
                    enable_face=enable_face,
                    enable_behavior=enable_behavior,
                    enable_uniformer=enable_uniformer,
                    enable_positioning=enable_positioning,
                ...
                # 按模块名排序以便于查阅
                for mod, t in sorted(self.profiler.latest_times.items()):
                    f.write(f"  - {mod:<25}: {t * 1000:.2f} ms\n")
                f.write("-" * 50 + "\n")

        cap.release()
        print(f"\n>>> 视频处理完毕！单帧耗时数据已保存至: {output_txt}")
        print("\n>>> 以下是各模块的全局性能评估报告：")
...
if __name__ == "__main__":
    ...
    # 【注意】请将此处替换为你实际测试用的视频路径，字符串前面加上 r 防止转义字符报错
    video_file = r"/home/ubuntu/eval_workspace/videos/NVR_ch3_main_20260409152140_20260409152409.mp4" 
    
    # 调用视频处理方法，根据需求开启或关闭相关功能模块
    service.process_video(
        video_path=video_file,
        camera_id="203",
        enable_face=True,         # 开启人脸识别
        enable_behavior=True,     # 开启行为识别（基础）
        enable_uniformer=False,    # 开启行为识别（Uniformer模型）
        enable_positioning=True,  # 开启世界坐标/姿态估计
        enable_tracking=True,     # 开启追踪和 ReID
        output_txt="frame_process_times.txt" # 设置输出 txt 文件名称
    )
